...
import predictability.obsolute.CollaboratePredictabilityWorld


class ItemPickWorldWeight(
    predictability.obsolute.CollaboratePredictabilityWorld.CollaboratePredictabilityWorld):
    def __init__(self):
    ...
    def make_pre_cond(self, pre_assign):
        return tuple([(pre[-1], self.single_orderd_assign_cost(pre, (self.goal_ids[-1] + i + 1, 0, i)))
    ...
    def orderd_assign_cost(self, o_assign, pre_cond, true_cost=0):
        return max(self.single_orderd_assign_cost(o_assign[0], pre_cond[0], true_cost),
                   self.single_orderd_assign_cost(o_assign[1], pre_cond[1], 0))
    ...
    def single_orderd_assign_cost(self, o_assign, pre_cond, true_cost=0):
        ...
        return sum([self.dists[assign[i], assign[i + 1]] for i in range(len(assign) - 1)]) + pre_cond[1] \
               + np.sum(self.weight[list(assign)]) * true_cost
