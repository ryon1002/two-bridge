...
from world import FieldMDP, TwoAgentMOMDP_Bridge
import numpy as np
...
class Application():
    def __init__(self, worldcsvfile):
        ...
        for i in range(len(self.bridge)):
            ...
            agent.setGoal(agent.shrinkMap[self.goal])
        ...
        self.collaborate = TwoAgentMOMDP_Bridge.TwoAgentMOMDP_Bridge(self.agents, self.sidekick, self.bridge, self.goal)
...
#         b1 = np.arange(0, 1.01, 0.1)
        ...
        self.collaborate.calcAvector(10, self.b, True)
...
#         self.collaborate.aVectorA = pickle.load(open('aVectorA.pkl', mode='r'))
    ...
    def move(self, event):
        if event.key in self.keyaMap:
            ...
            self.agentState = self.agents[0].getNextState(self.keyaMap[event.key], self.agentState)
            s_a = self.collaborate.getBestAction(self.collaborateState, self.belief)
            self.sidekickState = self.sidekick.getNextState(s_a, self.sidekickState)
            self.collaborateState = self.collaborate.getState(self.sidekickState, self.agentState)
